/* QA: full family creation via UI (v3 - correct role select) */
...
(async () => {
  ...
  // Role select = combobox currently showing "Anak"
  const roleCombo = page.locator('[role="combobox"]').filter({ hasText: /^Anak$/ }).first();
  await roleCombo.click();
  await page.waitForTimeout(700);
  const opts = page.locator('[role="option"]');
  const optTexts = await opts.allInnerTexts();
  console.log("role option texts:", JSON.stringify(optTexts));
